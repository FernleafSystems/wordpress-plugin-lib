...
class General extends Base {
	...
	public function getPluginDataAsObject( $sPluginBaseFilename ){
		...
		return Services::Data()->convertArrayToStdClass( $aPlugins[ $sPluginBaseFilename ] );
	}
	...
	public function getUrl_CurrentAdminPage() {
		...
		if ( $sPage == 'admin.php' ) {
			$sSubPage = Services::Data()->FetchGet( 'page' );
			if ( !empty( $sSubPage ) ) {
				$aQueryArgs = array(
					'page' 	=> $sSubPage,
				);
	...
	public function getIsLoginUrl() {
		...
		$aRequestParts = Services::Data()->getRequestUriParts();
	...
	public function setActivePluginLoadPosition( $sPluginFile, $nDesiredPosition = 0 ) {
		...
		$aActive = Services::Data()->setArrayValueToPosition( $this->getOption( 'active_plugins' ), $sPluginFile, $nDesiredPosition );
		$this->updateOption( 'active_plugins', $aActive );

		if ( $this->isMultisite() ) {
			$aActive = Services::Data()->setArrayValueToPosition( $this->getOption( 'active_sitewide_plugins' ), $sPluginFile, $nDesiredPosition );
	...
	public function getTimeStringForDisplay( $nTime = null, $bShowTime = true, $bShowDate = true ) {
		$nTime = empty( $nTime ) ? Services::Data()->time() : $nTime;
